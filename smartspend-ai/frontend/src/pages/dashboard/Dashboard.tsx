    ...
    <Card>
      <CardContent sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
          <FavoriteIcon sx={{ color, fontSize: 18 }} />
        ...
        <LinearProgress variant="determinate" value={score}
          sx={{ mt: 2, height: 6, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.06)',
...
export default function Dashboard() {
  ...
  const doughnutData = {
    ...
    datasets: [{ data: cats.map((c: any) => Number(c?.amount) || 0),
  ...
  const lineData = {
    ...
    datasets: [{
      label: 'Spending',
      data: [expenses * 0.2, expenses * 0.25, expenses * 0.3, expenses * 0.25],
    ...
    <Box>
      ...
      <Grid container spacing={2.5}>
        <Grid item xs={12} sm={6} lg={3}>
          <StatCard title="Monthly expenses" value={fmt(expenses)}
            subtitle="This month" color="#FF5C7C" IconComp={TrendingDownIcon} />
        </Grid>
        <Grid item xs={12} sm={6} lg={3}>
          <StatCard title="Monthly income" value={fmt(income)}
            subtitle="This month" color="#00D4AA" IconComp={TrendingUpIcon} />
        </Grid>
        <Grid item xs={12} sm={6} lg={3}>
          <StatCard title="Savings" value={fmt(savings)}
            subtitle={savingsRate + '% savings rate'} color="#6C63FF" IconComp={SavingsIcon} />
        </Grid>
        <Grid item xs={12} sm={6} lg={3}>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              ...
              {cats.length > 0 ? (
                <Doughnut data={doughnutData} options={{ plugins: {
        ...
        <Grid item xs={12}>
          <Card>
            <CardContent sx={{ p: 3 }}>
              ...
              {recent.length > 0 ? recent.map((e: any, i: number) => (
                <Box key={String(e?.id || i)} sx={{ display: 'flex', justifyContent: 'space-between',
